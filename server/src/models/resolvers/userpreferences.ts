...
export const userpreferenceResolvers = {
  Query: {
    listUserPreferences: async (parent: undefined, filter?: filtersorttype<userpreferencefiltertype>): Promise<userpreferencetype[]> => {
      try {
        const alluserpreferences: userpreferencetype[] = await listAll<userpreferencetype, userpreferencefiltertype>("user_preferences", filter );
