...
export const chatroomMutations = {
  Mutation: {
    insertChatroom: async(parent: undefined, { data }: chatroomdatatype, { pubsub }: any ): Promise<chatroomtype> => {
      try {      
        const revRoomCode: string = data.room_code.split("-").reverse().join("-");        
        ...
        if(!foundChatroom) {          
          ...
          throw new Error(`Chatroom already Exist id: ${foundChatroom.room_code}`);         
        }                
    ...
    softDeleteChatroom: async(parent: undefined, { data }: chatroomdatatype, { pubsub }: any): Promise<chatroomtype> => {
      try {      
        const foundChatroom: chatroomtype = await findOne<chatroomtype, { room_code: string }>("chatrooms", { "room_code": data.room_code });          
        if(!foundChatroom) throw new Error(`Chatroom not found id: ${ data.room_code }`);
        ...
        const [delUserChatroom]: ruserchatroomtype[] = await db("user_chatrooms")
    ...
    removeChatroom: async(parent: undefined, { data }: remchatroomdatatype): Promise<rchatroomtype> => {
      try {
        const foundChatroom: chatroomtype = await findOne("chatrooms", { "id": data.id });
