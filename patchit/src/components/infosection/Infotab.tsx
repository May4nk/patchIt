...
const Infotab = (infotabprops: infotabprops) => {
  ...
  const communitydp: string = (community?.profile_pic?.length > 0 && community?.profile_pic !== null)
    ? community?.profile_pic?.substr(12, ) 
    : "a.jpg";
        ...
        <Link to={`/c/${community.communityname}`} className="popularcommunity">
          <div className="popularcommunityheader">
            <div className="popularcommunitypicwrapper">
              <img src={ require(`../../img/${communitydp}`) } alt="community profile pic" className="popularcommunitypic"/>
            </div>
