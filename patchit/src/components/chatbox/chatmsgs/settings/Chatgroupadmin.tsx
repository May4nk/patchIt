...
import { changeToBase64 } from '../../../../utils/opx/opx';
...
import { CHATMEDIA } from '../../../../utils/main/types';
...
function Chatgroupadmin(chatgroupadminprops: chatgroupadminprops) {
  ...
  //handlers
  const handleGroupMedia: (mediaSetting: CHATMEDIA) => void = (mediaSetting: CHATMEDIA) => {
    updateRoomSettings({ ...roomSettings, allowedMedia: mediaSetting });
  };
  ...
  const handleUpdatePic: () => Promise<void> = async () => {
    if (roomProfileRef?.current && roomProfileRef?.current?.files) {
      ...
      if (pic) {
        updateRoomSettings({ ...roomSettings, profile_pic: pic })
      }
    }
  }
  ...
  const handleSettings: (e: any) => void = (e: any) => {
    updateRoomSettings({ ...roomSettings, [e.target.name]: e.target.value });
  }
