...
export function generateRoomCode(ids: number[], room: boolean): string {
  const userIdsString = JSON.stringify(ids);
  const encodedPayload = btoa(userIdsString);
  const timestamp = Date.now().toString(36);
  const randomString = Math.random().toString(36).substring(2, 8);
  let roomCode: string = "";
  if (room) {
    roomCode = `${timestamp}-${encodedPayload}-${randomString}`;
  } else {
    roomCode = `room-${encodedPayload}`;
  }
